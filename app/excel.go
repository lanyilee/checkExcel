...
func ReadExcel(path string) ([]model.ProvinceCity,error){
	...
	for rowIndex, row := range rows {
		...
		for colIndex, colCell := range row {
			switch colIndex {
			case 0: person.Number=colCell
			case 1: person.Name=colCell
			case 2: person.Sex=colCell
			case 3: person.Age=colCell
			case 4: person.Id=colCell
			case 5: person.Phone=colCell
			//计数村
			case 6: person.Village=colCell
			case 7: person.LeaveTime=colCell
			case 8: person.BackTime=colCell
			case 9: person.Control=colCell
			case 10: person.ControlTime=colCell
			case 11: person.PrevAddr=colCell
			}
			//fmt.Print(colCell, "\t")
		}
		selectCity:=""
...
func WriteBodyRight(f *excelize.File,f1SheetName string, path string,cities []model.ProvinceCity,villageNames []string)error{
	...
	for _,name:=range bigData.EnterpriseName{
		if bigData.EnterpriseMap[name].Flag==0{
			...
			//添加该企业一行数据
			for i,a:=range array{
				colNum:=getColNumber(cityLen,asciiC,i+2,len(villageNames)+6)
				if a!=0{
					f.SetCellInt(f1SheetName,colNum,a)
				}
			}
...
func ReadBigDataExcel(path string) (*model.BigData,error){
	...
	for rowIndex,row:=range f2rows{
		...
		if rowIndex>=6 && rowIndex<=16{  // 获取数据
			for colIndex, colCell := range row {
			...
			for colIndex, colCell := range row {
				switch colIndex {
				case 8:bigData.Transfer.Number=colCell
				case 9:bigData.Repeat.Number=colCell
				case 10:bigData.NeverCheck.Number=colCell
				case 11:bigData.Remark.Number=colCell
				}
			}
		}else if rowIndex>16 {                            //企业,先记录，记录企业名数组和对应的hash数组
			//企业常量数组
